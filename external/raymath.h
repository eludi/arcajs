...
RMDEF Matrix MatrixFrustum(double left, double right, double bottom, double top, double nr, double fr)
{
    Matrix result = { 0 };

    float rl = (float)(right - left);
    float tb = (float)(top - bottom);
    float fn = (float)(fr - nr);

    result.m0 = ((float) nr*2.0f)/rl;
    result.m1 = 0.0f;
    result.m2 = 0.0f;
    result.m3 = 0.0f;

    result.m4 = 0.0f;
    result.m5 = ((float) nr*2.0f)/tb;
    result.m6 = 0.0f;
    result.m7 = 0.0f;

    result.m8 = ((float)right + (float)left)/rl;
    result.m9 = ((float)top + (float)bottom)/tb;
    result.m10 = -((float)fr + (float)nr)/fn;
    result.m11 = -1.0f;

    result.m12 = 0.0f;
    result.m13 = 0.0f;
    result.m14 = -((float)fr*(float)nr*2.0f)/fn;
    result.m15 = 0.0f;
...
// Returns perspective projection matrix
// NOTE: Angle should be provided in radians
RMDEF Matrix MatrixPerspective(double fovy, double aspect, double nr, double fr)
{
    double top = nr*tan(fovy*0.5);
    double right = top*aspect;
    Matrix result = MatrixFrustum(-right, right, -top, top, nr, fr);

    return result;
}

// Returns orthographic projection matrix
RMDEF Matrix MatrixOrtho(double left, double right, double bottom, double top, double nr, double fr)
{
    Matrix result = { 0 };
    ...
    result.m14 = -((float)fr + (float)nr)/fn;
    result.m15 = 1.0f;
